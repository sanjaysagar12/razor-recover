...
import { useDashboardData, execTileTone } from '../data/useDashboardData.js';

export default function DashboardPage({ paused, setPaused }) {
  ...
  const {
    ...
    triggerCase,
    refreshAll,
  } = useDashboardData({ paused, toast });

  return (
    <div className="flex-1 min-w-0">
      <div className="flex items-end justify-between flex-wrap gap-5 px-7 pt-6 pb-2">
        <div className="flex flex-col gap-2.5">
          <h1 className="text-[34px] font-extrabold m-0 tracking-tight">Dashboard</h1>
          <div className="text-muted text-[13px]">Audit log &middot; pipeline decisions for cases processed this session</div>
        </div>
      ...
      <section className="px-7 pb-7 pt-2">
        <div className="bg-white border border-gray-100 rounded-2xl shadow-card p-[18px] mb-4">
          <div className="text-[11px] font-semibold text-muted uppercase tracking-wider mb-1">Guardrail edge cases</div>
          <h2 className="text-2xl font-extrabold text-black leading-tight mb-4">Preset scenarios</h2>
          <PresetPanel onTrigger={(payload) => triggerCase(payload)} />
          <CustomTriggerPanel onTrigger={(payload, endpoint) => triggerCase(payload, endpoint)} />
        </div>
